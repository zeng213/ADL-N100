  ...
  )
{
  ...
  if (GuidHob != NULL) {
    ...
    Status = PcdSet32S (PcdFspsBaseAddress, FspInfo->FspsBase);
    ASSERT_EFI_ERROR (Status);
  }

  //
  // Find the secure boot information and update PCDs
  //
  GuidHob = GetFirstGuidHob (&gSecureBootInfoGuid);
  if (GuidHob != NULL) {
    SecurebootInfo = (SECUREBOOT_INFO *)GET_GUID_HOB_DATA (GuidHob);
    Status = PcdSet32S (PcdTpm2HashMask, SecurebootInfo->TpmPcrActivePcrBanks);
    ASSERT_EFI_ERROR (Status);
    Status = PcdSetBoolS (PcdFirmwareDebuggerInitialized, SecurebootInfo->FirmwareDebuggerInitialized);
    ASSERT_EFI_ERROR (Status);
  }
