  ...
  )
{
  ...
  StackTop = (UINTN)PcdGet32 (PcdPayloadStackTop);
  ASSERT (StackTop > sizeof (UINT64));

  HandoffTable = (EFI_HOB_HANDOFF_INFO_TABLE *)(*(UINTN *)(StackTop - sizeof (UINT64)));

  DEBUG ((EFI_D_ERROR, "HandoffTable = 0x%p\n", HandoffTable));
