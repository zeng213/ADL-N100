/** @file
  ...
  Copyright (c) 2008 - 2023, Apple Inc. All rights reserved.<BR>
  ...
  )
{
  ...
  if (FvInstance != NULL) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      DEBUG ((EFI_D_ERROR, "FfsFindNextFile Status = %r\n", Status));
    }
  } else {
...
// For NT32 Debug  Status = SecWinNtPeiLoadFile (PeCoffImage, &ImageAddress, &ImageSize, &EntryPoint);
  ...
  //
  ZeroMem (&FvFileInfo, sizeof (EFI_FV_FILE_INFO));
  Status = FfsGetFileInfo (FileHandle, &FvFileInfo);
  ASSERT_EFI_ERROR (Status);

  BuildModuleHob (&FvFileInfo.FileName, (EFI_PHYSICAL_ADDRESS)(UINTN)ImageAddress, EFI_SIZE_TO_PAGES ((UINT32) ImageSize) * EFI_PAGE_SIZE, *EntryPoint);

  DEBUG ((EFI_D_INFO | EFI_D_LOAD, "Loading DxeCore at 0x%10p EntryPoint=0x%10x\n", (VOID *)(UINTN)ImageAddress, *EntryPoint));
