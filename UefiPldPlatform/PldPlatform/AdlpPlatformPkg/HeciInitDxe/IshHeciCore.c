...
#include "../Override/PldCommonPkg/Library/PlatformSupportLibSbl/DeviceTableHobGuid.h"
  ...
  )
{
  ...
  UINT8              *GuidHob;
  CONST VOID         *HobList;
  UINT32             IshDeviceNumber = PCI_DEVICE_NUMBER_PCH_ISH;

  HobList = GetParameterBase ();
  ASSERT (HobList != NULL);
  GuidHob = GetNextGuidHob (&gDeviceTableHobGuid, HobList);
  ASSERT (GuidHob != NULL);
  DeviceTable = (PLT_DEVICE_TABLE *) GET_GUID_HOB_DATA (GuidHob);

  if (DeviceTable == NULL) {
    DEBUG ((DEBUG_ERROR, "DeviceTable == NULL\n"));
  ...
  for (Index = 0; Index < DeviceTable->DeviceNumber; Index++) {
    ...
    if ((PltDevice->Type == PltDeviceIsh) ){
      IshDeviceNumber = PltDevice->Dev.PciDev.PciDeviceNumber;
      break;
    }
  }
  DEBUG ((DEBUG_INFO, "IshDeviceNumber == 0x%d\n", IshDeviceNumber));

  IshBaseAddress = PCI_SEGMENT_LIB_ADDRESS (ME_SEGMENT, ME_BUS, IshDeviceNumber, PCI_FUNCTION_NUMBER_PCH_ISH, 0);
  ...
  if (DevId != V_ADL_P_ISH_DEVICE_ID) {
    return EFI_DEVICE_ERROR;
