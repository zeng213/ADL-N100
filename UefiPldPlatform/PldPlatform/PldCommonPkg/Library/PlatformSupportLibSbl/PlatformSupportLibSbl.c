...
/**
  Acquire the TPM Hardware type info and update
  ...
  )
{
  ...
  *TpmAvaiable = 0;
  ...
  if(PlatformInfo->Revision > 1){
    if (PlatformInfo->TpmType == TPM_TYPE_NONE){
      DEBUG((EFI_D_INFO, "TpmType - TPM_TYPE_NONE \n"));
    } else if (PlatformInfo->TpmType == TPM_TYPE_DTPM20){
      *TpmAvaiable = 1;
      DEBUG((EFI_D_INFO, "TpmType - TPM_TYPE_DTPM20 \n"));
    } else if (PlatformInfo->TpmType == TPM_TYPE_PTT){
      *TpmAvaiable = 1;
      DEBUG((EFI_D_INFO, "TpmType - TPM_TYPE_PTT \n"));
    } else if (PlatformInfo->TpmType == TPM_TYPE_NOT_KNOWN){
      DEBUG((EFI_D_INFO, "TpmType - TPM_TYPE_NOT_KNOWN \n"));
    }
  }

  // Check for TPM availablity and set Firmware PCD
  if(*TpmAvaiable == 1) {
    SetFirmwareDebugPcd (PlatformDebugStateEnabled (PlatformInfo->HwState));
  }
