...
typedef struct {
  UINT32 Tcg2Lasa;
  UINT32 Tcg2LasaEvenLast;
  UINT32 Tcg2Laml;
} TPM_EVENT_LASA_INFO;

TPM_EVENT_LASA_INFO mTpmLasaInfo = {
  0,  // TPM Event log start address
  0,  // TPM Event log last address
  0,  // TPM Event log min length
};

/**
  ACPI  platform specific definition in Slimboot loader.
  These are  additional parameters to EFI_TPM2_ACPI_TABLE.
**/
typedef struct {
  UINT8                       PlatformSpecificParameters[12];   //Additional params in bootloader
  UINT32                      LogAreaMinimumLen;                //Additional params in bootloader - Laml
  UINT64                      LogAreaStartAddress;              //Additional params in bootloader - Lasa
} EFI_TPM2_ACPI_PLAT_INFO;
...
/**
  Get the Tpm 2 Acpi Table.

  @param[out] Tpm2AcpiTable   Double pointer to start of Tpm 2 Acpi table.

  @retval EFI_SUCCESS         Operation completed successfully.
  @retval others              A failure occurred.

**/
EFI_STATUS
GetTpm2AcpiTable (
  OUT EFI_TPM2_ACPI_TABLE **Tpm2AcpiTable
  )
{
  EFI_STATUS                                                Status;
  ...
  {
      UINT32 * Temp = (UINT32*)((UINT8*)Rsdt + sizeof(EFI_ACPI_DESCRIPTION_HEADER) + (sizeof(UINT32)*Index));
      Table =  (EFI_ACPI_DESCRIPTION_HEADER*)(UINTN)(*(Temp));
      if ((Table->Signature == EFI_ACPI_5_0_TRUSTED_COMPUTING_PLATFORM_2_TABLE_SIGNATURE )) {
        DEBUG ((EFI_D_INFO, "Found Tpm2 ACPI Table\n"));
        break;
      }
  }

  if (Index == NumEntries){
    DEBUG ((EFI_D_INFO, "Tpm2 ACPI Table not found\n"));
...
/**
  Set the Tpm Acpi Lasa and Laml.

  @param[in] Lasa       Log area start address.
  @param[in] Laml       Log area minimum length.
  ...
  )
{
  ...
  EFI_TPM2_ACPI_PLAT_INFO                                   *Tpm2AcpiPlatInfo;

  //
  // Get Tpm2 Acpi table
  //
  Status = GetTpm2AcpiTable (&Tpm2AcpiTable);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  // Set Lasa & Laml info
  //
  Tpm2AcpiPlatInfo = (EFI_TPM2_ACPI_PLAT_INFO *) ((UINT32) (UINTN)Tpm2AcpiTable + sizeof(EFI_TPM2_ACPI_TABLE));

  Tpm2AcpiPlatInfo->LogAreaMinimumLen = Laml;
  Tpm2AcpiPlatInfo->LogAreaStartAddress = Lasa;

  DEBUG ((EFI_D_INFO, "New TPM ACPI table info: Laml 0x%x Lasa 0x%x\n", Laml, Lasa));

  //
  // Update checksum
  //
  Tpm2AcpiTable->Header.Checksum = 0;
  Tpm2AcpiTable->Header.Checksum = CalculateCheckSum8 ((UINT8*)Tpm2AcpiTable, Tpm2AcpiTable->Header.Length);

  return EFI_SUCCESS;
}

/**
  Get the Tpm Acpi Lasa and Laml.

  @param[out] Lasa      Pointer to log area start address.
  @param[out] Laml      Pointer to log area minimum length.
...
{
  EFI_STATUS                                                 Status;
  EFI_TPM2_ACPI_TABLE                                       *Tpm2AcpiTable = NULL;
  EFI_TPM2_ACPI_PLAT_INFO                                   *Tpm2AcpiPlatInfo;
  ...
  //
  Status = GetTpm2AcpiTable (&Tpm2AcpiTable);
  if (EFI_ERROR (Status) && Tpm2AcpiTable != NULL) {
    return Status;
  }
  ...
  *Laml = Tpm2AcpiPlatInfo->LogAreaMinimumLen;
  *Lasa = (UINT32)Tpm2AcpiPlatInfo->LogAreaStartAddress;

  DEBUG ((EFI_D_INFO, "Existing TPM ACPI table info: Laml 0x%x Lasa 0x%x\n", *Laml, *Lasa));
    ...
    if ((mTcgDxeData.BsCap.SupportedEventLogs & mTcg2EventInfo[Index].LogFormat) != 0) {
      ...
      case EFI_TCG2_EVENT_LOG_FORMAT_TCG_2:
        ...
        Status = TcgDxeLogEvent (
                   mTcg2EventInfo[Index].LogFormat,
                   &TcgPcrEvent2,
                   sizeof(TcgPcrEvent2.PCRIndex) + sizeof(TcgPcrEvent2.EventType) + GetDigestListSize (&TcgPcrEvent2.Digest) + sizeof(TcgPcrEvent2.EventSize),
                   NewEventData,
                   NewEventHdr->EventSize
                   );
        if (Status != EFI_SUCCESS) {
...
/**
  Calculate the TCG log LasaEvenLast info.

  @param[out] Lasa  Address to Log area start address .
  @param[out] Laml  Log Area Min len .
  @param[out] LasaLastEvent  Pointer to the Last event logged.
  ...
  )
{
  ...
  if (Lasa == 0 || Laml == 0 ) {
    DEBUG ((DEBUG_WARN, "Invalid Tcg2Lasa and Tcg2Laml !!\n"));
    *LasaLastEvent = 0;
    return EFI_INVALID_PARAMETER;
  }
  ...
  while (EmptySlot < (TCG_PCR_EVENT2_HDR *) ((UINTN) Lasa + (UINTN) Laml - 1)) {
    EventSize = GetCompressedTCGEventSize (EmptySlot);
    if (EventSize == 0) {
      break;
    } else {
      *LasaLastEvent = (UINT32) (UINTN)EmptySlot;
  ...
  )
{
  ...
  if ( (!FspInfo) || (FspInfo->MeasuredBoot == 0)) {
    DEBUG((EFI_D_INFO, "Measure boot is not enabled in bootloader. \n"));
    return TRUE;
  }

  Status = GetTpmAcpiTableInfo (&Lasa, &Laml);
  if (Status != EFI_SUCCESS) {
    DEBUG((EFI_D_INFO, "TPM Acpi table not present. \n"));
    return TRUE;
  }

  Status = GetTpmLogLastEvent(Lasa, Laml, &LasaLastEvent);
  if (Status != EFI_SUCCESS || (Lasa == LasaLastEvent)) {
    DEBUG ((EFI_D_ERROR, "No log found in the log event, disable measure boot.\n"));
    return TRUE;
  }

  mTpmLasaInfo.Tcg2Lasa         = Lasa;
  mTpmLasaInfo.Tcg2LasaEvenLast = LasaLastEvent;
  mTpmLasaInfo.Tcg2Laml         = Laml;

  DEBUG ((EFI_D_INFO, "Tcg2Dxe: Tcg2Lasa 0x%x Tcg2LasaEvenLast 0x%x\n", mTpmLasaInfo.Tcg2Lasa, mTpmLasaInfo.Tcg2LasaEvenLast));
  ...
  )
{
  ...
  UINTN                           Index;
  VOID                            *LastEvent = NULL;
  UINT32                          LastEvtSize = 0;
  UINT32                          EvtLogMinusLastSize = 0;
  ...
  for (Index = 0; Index < sizeof(mTcg2EventInfo)/sizeof(mTcg2EventInfo[0]); Index++) {
    if ((mTcgDxeData.BsCap.SupportedEventLogs & mTcg2EventInfo[Index].LogFormat) != 0) {
      ...
      //
      // Sync new Lasa and Laml with Tpm Acpi table so
      // modifications to event log can be seen using it
      //
      if (mTcg2EventInfo[Index].LogFormat == EFI_TCG2_EVENT_LOG_FORMAT_TCG_2) {
        SetTpmAcpiTableInfo (mTcgDxeData.EventLogAreaStruct[Index].Lasa, (UINT32)mTcgDxeData.EventLogAreaStruct[Index].Laml);
      }
    }
  }
  ...
  //
  // 3. Sync data from bootloader coreboot/SBL to DXE
  //
  Status = EFI_SUCCESS;
    ...
    if ((mTcgDxeData.BsCap.SupportedEventLogs & mTcg2EventInfo[Index].LogFormat) != 0) {
      switch (mTcg2EventInfo[Index].LogFormat) {
        case EFI_TCG2_EVENT_LOG_FORMAT_TCG_2:
          ...
          DEBUG((EFI_D_INFO, "Copying event logs in TPM2.0 format from bootloader\n"));
          if (mTpmLasaInfo.Tcg2Lasa && mTpmLasaInfo.Tcg2LasaEvenLast) {
            DumpEventLog (EFI_TCG2_EVENT_LOG_FORMAT_TCG_2, mTpmLasaInfo.Tcg2Lasa, mTpmLasaInfo.Tcg2LasaEvenLast, NULL);

            if (mTpmLasaInfo.Tcg2Lasa != mTpmLasaInfo.Tcg2LasaEvenLast)
            {
              LastEvtSize = (UINT32)GetPcrEvent2Size ((TCG_PCR_EVENT2*)(UINTN)(mTpmLasaInfo.Tcg2LasaEvenLast));
            }
            else
            {
              LastEvtSize = (UINT32)GetTcgEfiSpecIdEventStructSize ( (TCG_EfiSpecIDEventStruct *)(UINTN)(mTpmLasaInfo.Tcg2LasaEvenLast) );
            }
            EvtLogMinusLastSize = mTpmLasaInfo.Tcg2LasaEvenLast -  mTpmLasaInfo.Tcg2Lasa;

            CopyMem((void *)(mTcgDxeData.EventLogAreaStruct[Index].Lasa), (void *)(UINTN)(mTpmLasaInfo.Tcg2Lasa), EvtLogMinusLastSize + LastEvtSize );
            mTcgDxeData.EventLogAreaStruct[Index].EventLogStarted = TRUE;

            LastEvent = (UINT8 *)mTcgDxeData.EventLogAreaStruct[Index].Lasa + EvtLogMinusLastSize;

            mTcgDxeData.EventLogAreaStruct[Index].LastEvent = LastEvent;
            mTcgDxeData.EventLogAreaStruct[Index].EventLogSize = EvtLogMinusLastSize + LastEvtSize;
          }
          break;
      }
    }
...
/*
  // Disable TPM init since coreboot should already finish it.
  Status = Tpm2Startup (TPM_SU_CLEAR);
  DEBUG ((EFI_D_ERROR, "Tpm2Startup Status = %r\n", Status));

  Status = Tpm2SelfTest (NO);
  DEBUG ((EFI_D_ERROR, "Tpm2SelfTest Status = %r\n", Status));
*/
