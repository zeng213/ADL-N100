  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "MemInfoCallback: Base=0x%8lx, Size=0x%8lx, Type=0x%x (0x%x)\n", Base, Size, Type, MemoryMapEntry->Type));
  //
  // Build memory allocation HOB to report ACPI memory
  //
  ...
  if (Base >= 0x100000) {
    if (Type == EFI_RESOURCE_SYSTEM_MEMORY) {
      if (Base < 0x100000000ULL) {
		if ((MemoryMapEntry->Type != 3) && (MemoryMapEntry->Type != 4)) {
